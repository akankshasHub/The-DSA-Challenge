/* Leet Code Problem 
Link to problem description - https://leetcode.com/problems/frog-jump/description/ */
...
public:
    ...
    bool canCross(vector<int>& stones) {
        ...
        return solve(last_stone,stones[1],1);
    }
    bool solve(int last_stone, int curr_stone, int k)
    {
        ...
        {
            ans=ans||solve(last_stone,curr_stone+k-1,k-1);
        }
        if(s.find(curr_stone+k)!=s.end())
        {
            ans=ans||solve(last_stone,curr_stone+k,k);
        }
